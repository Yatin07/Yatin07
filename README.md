<img width="100%" src="https://raw.githubusercontent.com/halfrost/halfrost/master/icons/header_.png"/>

<h1 align="center">
  <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="28">
  Hey, I'm Yatin Patil
</h1>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=700&size=20&duration=3000&pause=800&color=00D9FF&center=true&vCenter=true&width=500&lines=4th+Year+B.Tech+IT+%40+NMIMS;Data+Science+%7C+AI%2FML+%7C+Android;194%2B+LeetCode+Problems+Solved+%E2%9C%85;Currently+building+AI+for+food+businesses+%F0%9F%8D%BD%EF%B8%8F"/>
</p>

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Yatin07&bg_color=0d1117&color=00D9FF&line=00D9FF&point=ffffff&area=true&hide_border=true" width="100%"/>
</div>

<div align="center">
  <img src="https://github-stats-alpha.vercel.app/api?username=Yatin07&cc=0d1117&tc=00D9FF&ic=00D9FF&bc=0d1117" height="160"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/username=Yatin07&layout=compact&theme=github_dark&hide_border=true&bg_color=0d1117&title_color=00D9FF&text_color=c9d1d9&cache_seconds=1800" height="160"/>
</div>

<br/>

---

### `$ whoami`

```yaml
name       : Yatin Patil
role       : B.Tech IT Student · Data Science & AI/ML Engineer
university : NMIMS University, Mumbai  |  CGPA: 3.58 / 4.0
batch      : 2023 – 2027  (4th Year)

now_building:
  - "AI Demand Forecasting Platform for small food businesses"
  - "Flutter + FastAPI + XGBoost/LSTM stack"

interests  : [ AI for real-world impact, Hackathons, Smart City Tech ]
superpower : Turning messy data into decisions that actually make sense
```

---

### 🔥 What I'm Working On

> **🍽️ AI-Powered Inventory Intelligence Platform** *(active)*
> Predicts daily/weekly product demand for restaurants & cloud kitchens using historical sales, weather, festivals & seasonality.
> Stack: `Flutter` · `FastAPI` · `XGBoost / LightGBM / LSTM` · `PostgreSQL` · `OpenWeather API`

---

### 🛠 Projects

| Project | What it does | Stack |
|--------|-------------|-------|
| 🌾 **[Agro.ai](https://github.com/Yatin07)** | Crop disease detection on 87K+ images, 38 classes, MobileNetV2 + CNN pipeline | TensorFlow · Scikit-learn · Python |
| 📊 **[ATP Analytics Dashboard](https://github.com/Yatin07)** | Power BI dashboard + XGBoost match win predictor, Galaxy Schema data model | Power BI · XGBoost · DAX |
| 📱 **[Hostel CMS](https://github.com/Yatin07)** | Multi-role Android app with real-time Firestore, RBAC, image compression, Python automation | Java · Firebase · Android Studio |
| 🍽️ **[Demand Forecast Platform](https://github.com/Yatin07)** *(WIP)* | ML-driven stock recommendations for small food businesses | Flutter · FastAPI · PostgreSQL |

---

### ⚡ Tech I Reach For

<table>
<tr><td><b>Languages</b></td><td>

![Python](https://img.shields.io/badge/Python-black?style=flat-square&logo=python)
![Java](https://img.shields.io/badge/Java-black?style=flat-square&logo=openjdk)
![SQL](https://img.shields.io/badge/SQL-black?style=flat-square&logo=mysql)
![JavaScript](https://img.shields.io/badge/JavaScript-black?style=flat-square&logo=javascript)
![R](https://img.shields.io/badge/R-black?style=flat-square&logo=r)

</td></tr>
<tr><td><b>AI / ML</b></td><td>

![TensorFlow](https://img.shields.io/badge/TensorFlow-black?style=flat-square&logo=tensorflow)
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-black?style=flat-square&logo=scikit-learn)
![XGBoost](https://img.shields.io/badge/XGBoost-black?style=flat-square)
![Pandas](https://img.shields.io/badge/Pandas-black?style=flat-square&logo=pandas)
![NumPy](https://img.shields.io/badge/NumPy-black?style=flat-square&logo=numpy)

</td></tr>
<tr><td><b>Analytics</b></td><td>

![Power BI](https://img.shields.io/badge/Power_BI-black?style=flat-square&logo=powerbi)
![Tableau](https://img.shields.io/badge/Tableau-black?style=flat-square&logo=tableau)
![Plotly](https://img.shields.io/badge/Plotly-black?style=flat-square&logo=plotly)

</td></tr>
<tr><td><b>Dev & Cloud</b></td><td>

![Flutter](https://img.shields.io/badge/Flutter-black?style=flat-square&logo=flutter)
![FastAPI](https://img.shields.io/badge/FastAPI-black?style=flat-square&logo=fastapi)
![Firebase](https://img.shields.io/badge/Firebase-black?style=flat-square&logo=firebase)
![Node.js](https://img.shields.io/badge/Node.js-black?style=flat-square&logo=node.js)
![GCP](https://img.shields.io/badge/GCP-black?style=flat-square&logo=google-cloud)

</td></tr>
<tr><td><b>Databases</b></td><td>

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-black?style=flat-square&logo=postgresql)
![MySQL](https://img.shields.io/badge/MySQL-black?style=flat-square&logo=mysql)
![MongoDB](https://img.shields.io/badge/MongoDB-black?style=flat-square&logo=mongodb)
![Firestore](https://img.shields.io/badge/Firestore-black?style=flat-square&logo=firebase)

</td></tr>
</table>

---

### 📈 GitHub Activity

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Yatin07&bg_color=0d1117&color=00D9FF&line=00D9FF&point=ffffff&area=true&hide_border=true" width="100%"/>
</div>

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Yatin07&show_icons=true&theme=github_dark&hide_border=true&bg_color=0d1117&title_color=00D9FF&icon_color=00D9FF&text_color=c9d1d9" height="160"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Yatin07&layout=compact&theme=github_dark&hide_border=true&bg_color=0d1117&title_color=00D9FF&text_color=c9d1d9" height="160"/>
</div>

---

### 🧩 LeetCode — 200+ Problems Solved

<div align="center">
  <img src="https://leetcard.jacoblin.cool/Yatin07?theme=dark&font=JetBrains+Mono&ext=heatmap&border=0&radius=8"/>
</div>

---

### 🏅 Highlights

```
🥇  SAS Curiosity Cup — Global Data Analytics Competition
⚡  Buildathon 2026 + multiple national-level hackathons
🎓  CGPA 3.58 / 4.0  |  IBM SkillsBuild Data Analytics Intern (2024)
🐱  My cat has reviewed more of my commits than my professors
```

---

<div align="center">
  <sub>💬 Open to internships · research collabs · hackathon teams · cool ideas → <a href="mailto:yatinpatilyp07@gmail.com">yatinpatilyp07@gmail.com</a></sub>
</div>
